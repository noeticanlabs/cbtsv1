...
class GRPhaseLoomOrchestrator:
    ...
    def run_step(self, dt_max=None):
        ...
        # Fix dt_thread['phys'] to dt_cfl
        dt_cfl = self.threads.C_CFL * self.fields.dx / 1.0  # c=1
        proposals['phys']['dt'] = dt_cfl
        ...
        violation = self.stepper.memory.honesty_check(eps_H + eps_M, eps_H_post + eps_M_post, self.rails, eps_H_post, eps_M_post, self.geometry, self.fields)
        if violation:
            self.tainted = True
            self.rollback_count += 1
            self.rollback_reason = f"Tainted by honesty check: {violation}"
        ...
        self.receipts.emit_receipt(self.step, self.t, dt, dominant_thread, proposals, eps_H, eps_M, eps_H_post, eps_M_post, d_eps_H, d_eps_M, max_R, m_det, self.threads.mu_H, self.threads.mu_M, self.rollback_count, self.rollback_reason, loom_data, t_expected, t_err, self.dt_loom_prev, risk_gauge, tight_threads, consistency_ok, rail_margins, lambda_min, lambda_max, cond_gamma, repair_applied, repair_type, lambda_min_pre, lambda_min_post, t_prev, self.t, dt, dt_applied, substeps, True)
