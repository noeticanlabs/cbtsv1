...
class VMError(Exception):
    ...
    def __init__(self, message: str, ip: int = -1, instruction: Instruction = None):
...
class VirtualMachine:
    ...
    def _handle_grad(self, inst: Instruction):
        """Compute gradient: ∇f"""
        field_name = self._pop().value if self._peek().value_type == ValueType.FIELD else None
    ...
    def _handle_check_invariant(self, inst: Instruction):
        ...
        if inv_id in self.invariants:
            expected, tolerance = self.invariants[inv_id]
            if abs(value - expected) > tolerance:
                raise InvariantViolationError(inv_id, value, expected, self._ip)
