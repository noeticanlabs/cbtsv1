...
def test_gcat2_scenario2_constraint_violating_perturbation():
    ...
    # Evolve 10 time units
    solver.run(T_max=10.0)
    ...
    damping = (eps_H_initial - eps_H_final_early) / (eps_H_initial + 1e-15) > 0.1 # Expect some damping

    # No explosion: fields bounded
    bounded = all(np.all(np.abs(solver.fields.gamma_sym6) < 1e6) and
                  np.all(np.abs(solver.fields.K_sym6) < 1e6))
    ...
    summary = {
        ...
        "metrics": {
            ...
            "eps_H_final_20": eps_H_final_early,
            "bounded": bounded
        },
