"""
MMS Convergence Test - Time Integration Analysis
Tests whether error floor is from time integration (dt too large) or spatial discretization.
"""
import numpy as np
import logging
import sys
...
def run_test(N, L=16.0, T=0.00001, dt_factor=0.01):
    """Run test with very small dt to isolate spatial error."""
    ...
    dx = L / N
    dt = (dx**2) * dt_factor  # Very small dt
    t = 0.0
...
def main():
    ...
    logger.info("=== MMS Error vs dt_factor (smaller dt = less time error) ===")
    logger.info(f"N={N}, T={T}")
    logger.info("")
    
    for dt_factor in [0.5, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002]:
        result = run_test(N, L=L, T=T, dt_factor=dt_factor)
        logger.info(f"dt_factor={dt_factor:4.2f}: Error={result['total_error']:.4e}, steps={result['steps']}")
        
        # Stop if error starts increasing (numerical issues)
        if dt_factor < 0.01 and result['total_error'] > 1e-6:
            logger.warning("Error growing - dt too small, numerical issues")
            break
