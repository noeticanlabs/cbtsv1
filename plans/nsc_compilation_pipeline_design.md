...
- Compilation Flow: High-level NSC compilation (via `noetica_nsc_phase1/nsc.py`) now optionally targets IR export instead of direct bytecode. New compiler script (to be created) orchestrates IR → PIR → Hadamard bytecode. Existing bytecode paths remain for fallback.
